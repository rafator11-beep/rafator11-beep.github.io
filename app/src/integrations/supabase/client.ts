...
class MockQueryBuilder {
  ...
  match() { return this; }
  maybeSingle() { 
    console.log("Mock Supabase: maybeSingle()", this.result);
    return Promise.resolve(this.result); 
  }
  single() { 
    console.log("Mock Supabase: single()", this.result);
    if (!this.result.data && !this.result.error) {
      // Return a slightly more helpful error if nothing was provided
      return Promise.resolve({ data: null, error: new Error("Mock: No data found for single()") });
    }
    return Promise.resolve(this.result); 
  }
...
const SUPABASE_PUBLISHABLE_KEY =
  ...
  // Use a safer key name or encourage ENV
